...
void Judge::BattleIsJudge(Player& player_,Enemy& enemy_,Skill& skill_)
{
	if (player_.attck != 0 && enemy_.attck != 0)
	{
		...
		if (skill_.isRevolution == true)
		{
			...
			{
				player_.hp -= player_.attck;
			}
		}
	}
}

void Judge::BattleReset(Player& player_,Enemy& enemy_,card& card_,Skill& skill_)
{
  	player_.isTurn = false;
	enemy_.isTurn = false;
	player_.attck = 0;
	enemy_.attck = 0;
	card_.isT = false;
	skill_.isRevolution = false;
	//enemy_.isCard = false;
	//player_.isCard = false;
}

void Judge::BattleUpdate(Player& player_,Enemy& enemy_,card& card_, Skill& skill_)
{
	if (player_.isTurn == true && enemy_.isTurn == true)
	{
		enemy_.isCard = true;
		player_.isCard = true;
		BattleIsJudge(player_,enemy_,skill_);
		BattleReset(player_,enemy_,card_,skill_);
	}
	else
	{
